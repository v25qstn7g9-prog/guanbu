/**
 * functions/ask.js — 觀卜 AI 白話說明與對話 API（支援雙語切換 + Gemini 備援）
 */
const ASK_VERSION = "guanbu-ask-2.2-i18n";
...
4. 長度與排版：字數控制在 160 ~ 260 字左右（英文控制在 100 ~ 150 words），分 2-3 個短段落。
5. 語言回應規定：若 Prompt 指示語言為英文 (en)，請全程以溫暖自然的英文 (English) 回應；若為中文 (zh)，則以繁體中文回應。`;
...
3. 精煉流暢：每次回覆控制在 100 ~ 180 字之間（英文 60 ~ 100 words）。
4. 語言回應規定：請根據指示使用指定語言（English 或 繁體中文）回答。`;

function buildUserPrompt(mod, question, factsText, lang) {
  const isEn = lang === "en";
  const qLine = question 
    ? (isEn ? `User's Question: "${question}"\n\n` : `使用者目前的心情或問題是：「${question}」\n\n`)
    : (isEn ? "No specific question provided. Provide a general reading.\n\n" : "使用者未輸入特定問題，進行整體指引解析。\n\n");
  
  const modLabel = { yijing: "I Ching", tarot: "Tarot", runes: "Runes", ziwei: "Zi Wei Dou Shu", daily: "Daily Fortune" }[mod] || "Divination";
  const langInstruct = isEn ? "IMPORTANT: Please reply entirely in English." : "請以繁體中文回應。";

  return `【Type】: ${modLabel}\n${qLine}【Facts Data】:\n${factsText}\n\n${langInstruct}`;
}
